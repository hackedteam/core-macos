...
# macho_cstring_dissects.sh: split and delete macho cstring sects
# 
...
#  string not deleted in the __cstring section 
...
if [ $OFFSET == "0" ]
then
	#echo "otool -l $FILE | grep __cstring -A 5 | sed -n 's/offset //p'"
	TMP_OFFSET=`otool -l $FILE | grep __cstring -A 5 | sed -n 's/offset //p'`

	OFFSET=`printf "%d" $TMP_OFFSET`

	#echo "otool -l $FILE | grep __cstring -A 5 | sed -n 's/size //p'"
	HEX_SIZE=`otool -l $FILE | grep __cstring -A 5 | sed -n 's/size //p'`
	...
	FILE_NAME=`echo $1 | sed -n 's/--.*//p'`

	let "BEGIN_A = $OFFSET"
fi

echo filename = $1 offset = $OFFSET size = $SIZE
