    ...
    void **originalFunctionReentryIsland )
{
	...
	if (!err) {
#if defined(__i386__)
		uint32_t addressOffset = ((uint32_t)escapeIsland - (uint32_t)originalFunctionPtr - 5);
#elif defined(__x86_64__)
    uint64_t addressOffset = ((uint64_t)escapeIsland - (uint64_t)originalFunctionPtr - 5);
#endif
